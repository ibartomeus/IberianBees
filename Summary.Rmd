...
We have records all over the peninsula, but higer concentrations are marked in yellow.  
...
#code courtesi of Paco Rodriguez.
library(sf)
library(raster)
library(rasterVis)
...
dat <- subset(data, !is.na(Latitude))
...
ras.chao <- rasterize(datos.laea[, "accepted_name"], ras2,
               field = "accepted_name", fun = function(x, ...)
                 {estimateR(t(as.matrix(x = table(x), nrow = 1, ncol = x)))[2,1]} ) #2 chao, 4 ACE #TO THIS WELL, We nees to recover abundances.
